...
def minimax(depth, board, maximizing, ai_player, current_player):
    ...
    if maximizing:
        ...
        for c in valid_columns:
            ...
            temp_score, _= minimax(depth-1, temp_board, False, ai_player, opponent)
        ...
        for c in valid_columns:
            temp_board=clone_board(board)
            temp_board.drop_piece(c, current_player)
            temp_score, _= minimax(depth-1, temp_board, True, ai_player, ai_player)
            if temp_score<best_score:
                best_score=temp_score
                best_column=c
